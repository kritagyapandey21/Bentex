/**
 * Deterministic OHLC candle generator
 * CRITICAL: Must produce identical results on server and client
 */

import { createSeededRNG, gaussian } from './rng.js';

/**
 * Generate a single deterministic candle
 * @param {Object} params
 * @param {string} params.seedBase - Base seed string (symbol|timeframe|version|dateRange)
 * @param {number} params.index - Candle index
 * @param {number} params.prevClose - Previous candle's close price
 * @param {number} params.volatility - Volatility factor (e.g., 0.02 for 2%)
 * @param {number} params.timeframeMinutes - Timeframe in minutes
 * @param {number} params.priceDecimals - Decimal places for rounding (2-6)
 * @param {number} params.startTimeMs - Candle start time in UTC milliseconds
 * @returns {Object} Candle with {start_time_ms, open, high, low, close, volume}
 */
export function generateDeterministicCandle({
  seedBase,
  index,
  prevClose,
  volatility = 0.02,
  timeframeMinutes = 1,
  priceDecimals = 2,
  startTimeMs,
}) {
  // Seed for close price movement
  const closeSeed = `${seedBase}|candle|${index}`;
  const closeRng = createSeededRNG(closeSeed);

  // Generate Gaussian random variable for price movement
  const z = gaussian(closeRng);

  // Calculate percentage move
  const pctMove = z * volatility * Math.sqrt(timeframeMinutes);

  // Calculate OHLC
  const close = prevClose * (1 + pctMove);
  const open = prevClose;

  // Intraday high/low factors (deterministic)
  const intradaySeed = `${seedBase}|candle|${index}|intraday`;
  const intradayRng = createSeededRNG(intradaySeed);

  const intradayHighFactor = Math.abs(gaussian(intradayRng)) * volatility * 0.3;
  const intradayLowFactor = Math.abs(gaussian(intradayRng)) * volatility * 0.3;

  const high = Math.max(open, close) * (1 + intradayHighFactor);
  const low = Math.min(open, close) * (1 - intradayLowFactor);

  // Deterministic volume
  const volumeSeed = `${seedBase}|candle|${index}|volume`;
  const volumeRng = createSeededRNG(volumeSeed);
  const baseVolume = 100;
  const volume = Math.floor(baseVolume * (1 + volumeRng() * 0.5));

  // Round to specified decimals
  const round = (num) => Number(num.toFixed(priceDecimals));

  return {
    start_time_ms: startTimeMs,
    open: round(open),
    high: round(high),
    low: round(low),
    close: round(close),
    volume,
  };
}

/**
 * Generate a series of deterministic candles
 * @param {Object} params
 * @param {string} params.symbol - Trading symbol
 * @param {number} params.timeframeMinutes - Timeframe in minutes
 * @param {string} params.version - History version (e.g., "v1")
 * @param {number} params.startTimeMs - Start time in UTC milliseconds
 * @param {number} params.count - Number of candles to generate
 * @param {number} params.initialPrice - Starting price
 * @param {number} [params.volatility] - Volatility factor
 * @param {number} [params.priceDecimals] - Decimal places
 * @param {string} [params.dateRangeStartISO] - Optional date range for seed
 * @returns {Array} Array of candles
 */
export function generateSeries({
  symbol,
  timeframeMinutes,
  version,
  startTimeMs,
  count,
  initialPrice,
  volatility = 0.02,
  priceDecimals = 2,
  dateRangeStartISO = '',
}) {
  const seedBase = `${symbol}|${timeframeMinutes}|${version}|${dateRangeStartISO}`;
  const candles = [];
  let prevClose = initialPrice;

  const timeframeMs = timeframeMinutes * 60 * 1000;

  for (let i = 0; i < count; i++) {
    const candleStartTimeMs = startTimeMs + i * timeframeMs;

    const candle = generateDeterministicCandle({
      seedBase,
      index: i,
      prevClose,
      volatility,
      timeframeMinutes,
      priceDecimals,
      startTimeMs: candleStartTimeMs,
    });

    candles.push(candle);
    prevClose = candle.close;
  }

  return candles;
}

/**
 * Generate partial (forming) candle with deterministic interpolation
 * @param {Object} params
 * @param {string} params.seedBase - Base seed string
 * @param {number} params.index - Candle index
 * @param {number} params.prevClose - Previous candle's close
 * @param {number} params.candleStartMs - Candle start time in ms
 * @param {number} params.serverTimeMs - Current server time in ms
 * @param {number} params.timeframeMs - Timeframe in milliseconds
 * @param {number} [params.volatility] - Volatility
 * @param {number} [params.timeframeMinutes] - Timeframe in minutes
 * @param {number} [params.priceDecimals] - Decimal places
 * @returns {Object} Partial candle with isPartial: true
 */
export function generatePartialCandle({
  seedBase,
  index,
  prevClose,
  candleStartMs,
  serverTimeMs,
  timeframeMs,
  volatility = 0.02,
  timeframeMinutes = 1,
  priceDecimals = 2,
}) {
  // Generate target candle (what it will be when completed)
  const targetCandle = generateDeterministicCandle({
    seedBase,
    index,
    prevClose,
    volatility,
    timeframeMinutes,
    priceDecimals,
    startTimeMs: candleStartMs,
  });

  // Calculate elapsed fraction
  const elapsed = serverTimeMs - candleStartMs;
  const f = Math.min(1, Math.max(0, elapsed / timeframeMs));

  // Interpolate close
  const open = prevClose;
  const curClose = open + (targetCandle.close - open) * f;

  // Deterministic high/low for partial
  const intradaySeed = `${seedBase}|candle|${index}|intraday`;
  const intradayRng = createSeededRNG(intradaySeed);

  const intradayHighFactor = Math.abs(gaussian(intradayRng)) * volatility * 0.3 * f;
  const intradayLowFactor = Math.abs(gaussian(intradayRng)) * volatility * 0.3 * f;

  const curHigh = Math.max(open, curClose) * (1 + intradayHighFactor);
  const curLow = Math.min(open, curClose) * (1 - intradayLowFactor);

  const round = (num) => Number(num.toFixed(priceDecimals));

  return {
    start_time_ms: candleStartMs,
    open: round(open),
    high: round(curHigh),
    low: round(curLow),
    close: round(curClose),
    volume: Math.floor(targetCandle.volume * f),
    isPartial: true,
  };
}

/**
 * Compute candle index from UTC timestamp
 * @param {number} timeMs - UTC milliseconds
 * @param {number} timeframeMinutes - Timeframe in minutes
 * @returns {number} Candle index from UTC midnight
 */
export function getCandleIndex(timeMs, timeframeMinutes) {
  const timeframeMs = timeframeMinutes * 60 * 1000;
  return Math.floor(timeMs / timeframeMs);
}

/**
 * Get candle start time from index
 * @param {number} index - Candle index
 * @param {number} timeframeMinutes - Timeframe in minutes
 * @returns {number} Start time in UTC milliseconds
 */
export function getCandleStartTime(index, timeframeMinutes) {
  const timeframeMs = timeframeMinutes * 60 * 1000;
  return index * timeframeMs;
}
