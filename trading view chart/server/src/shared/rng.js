/**
 * Deterministic RNG utilities - identical implementation for server & client
 * CRITICAL: Do not modify without bumping history_version
 */

/**
 * xmur3 string hash - produces seed generator
 */
export function xmur3(str) {
  for (var i = 0, h = 1779033703; i < str.length; i++) {
    h = Math.imul(h ^ str.charCodeAt(i), 3432918353);
    h = (h << 13) | (h >>> 19);
  }
  return function () {
    h = Math.imul(h ^ (h >>> 16), 2246822507);
    h = Math.imul(h ^ (h >>> 13), 3266489909);
    return (h ^= (h >>> 16)) >>> 0;
  };
}

/**
 * sfc32 PRNG - simple fast counter
 */
export function sfc32(a, b, c, d) {
  return function () {
    a >>>= 0;
    b >>>= 0;
    c >>>= 0;
    d >>>= 0;
    var t = (a + b) | 0;
    a = b ^ (b >>> 9);
    b = (c + (c << 3)) | 0;
    c = (c << 21) | (c >>> 11);
    d = (d + 1) | 0;
    t = (t + d) | 0;
    c = (c + t) | 0;
    return (t >>> 0) / 4294967296;
  };
}

/**
 * Box-Muller transform for Gaussian distribution
 */
export function gaussian(rng) {
  const u1 = rng() || 1e-12;
  const u2 = rng() || 1e-12;
  return Math.sqrt(-2 * Math.log(u1)) * Math.cos(2 * Math.PI * u2);
}

/**
 * Create a seeded RNG from a string seed
 */
export function createSeededRNG(seedString) {
  const seed = xmur3(seedString);
  return sfc32(seed(), seed(), seed(), seed());
}
